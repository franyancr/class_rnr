...
int background_functions(
  ...
  /* KG-EDIT scalar field quantities FYCR-QDM */
	/* added delta_phi */
  double phi, phi_prime, delta_phi;
...
int background_solve(
  ...
  if (pba->background_verbose > 2) {
    ...
    if (pba->has_scf == _TRUE_){
      ...
      /* KG-EDIT FYCR-QDM */
	/* parameters: f_inj, log(1+zc), n, V_2, phi_prime_i*/
      printf("     -> parameters: [f_inj, log(1+zc), n, V_2, phi_prime_i] = \n");
      printf("                    [");
...
int background_initial_conditions(
  ...
  */
  ...
  /*KG-EDIT FYCR-QDM */
  /** Save rho_rad initial here to compute approximate scalar field value range later */
  pba->rho_rad_0 = rho_rad;
   ...
   *
     *
   * Modified initial condition to tracker solution - DP
   * FYCR-QDM Add approximate energy density contribution to rho_rad from the scalar field
   */
  if(pba->has_scf == _TRUE_){
    /*KG-EDIT FYCR-QDM PA
     * removed all attractor_ic_scf condition here, since it doesn't
     * make sense for phi^2n potentials
     */
...
int background_derivs(
  ...
  if (pba->has_scf == _TRUE_){
    ...
    /* KG-EDIT FYCR-QDM */
    /* Adding the coupling term to dark matter in the EOM */
...
double V_e_scf(struct background *pba,
  ...
  /* Here the parametrization is under the assumption that initial
   * phi-prime = 0, that the energy density injection is small
   * compared to LCDM, and that the energy is injected in matter
   * domination. Nothing should go wrong if these conditions are not
   * satisfied, but the parameters fede and tauc might not be close to
   * the peak energy injection and injection time. */

  double fede = pba->scf_parameters[0];
  double tauc = -pba->scf_parameters[1]; //tau = log(a)
  double n    = pba->scf_parameters[2];
  double V_2  = pba->scf_parameters[3];

  double rhoeq = 4124.0; //in Mpl^2/Mpc^2 units
  double taueq = -8.13; //tau = log(a)
  double rhoc = rhoeq*(
    0.5*exp(-3*(tauc-taueq))+
    0.5*exp(-4*(tauc-taueq)));

  double phi_initial = sqrt(0.6667*fede*n*(2*n-1.0));

  return  fede*rhoc*pow(pow(phi/phi_initial,2),n) + V_2;
}
...
double dV_e_scf(struct background *pba,
  ...
  //assuming n >=1 with phi^2n potential
  if(phi==0) 
    return 0;
  else
    return  fede*rhoc*(2*n)*pow(pow(phi/phi_initial,2),n)/phi;
}
...
double ddV_e_scf(struct background *pba,
  ...
  if(phi==0) 
    ...
    return  fede*rhoc*(2*n)*(2*n-1.0)*pow(pow(phi/phi_initial,2),n)/(phi*phi);
}
