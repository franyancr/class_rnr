...
int input_read_parameters(
  ...
  */
  ...
  if (pba->Omega0_scf != 0.){
    ...
    /*KG-EDIT
     * attractor_ic_scf must be set to FALSE, otherwise we don't
     * know what to do in the phi^2n case
     */
    if (flag1 == _TRUE_){
      class_test(((strstr(string1,"y") != NULL) || 
                  (strstr(string1,"Y") != NULL)),
               errmsg,
               "There are no attractor conditions available for "
               "phi^{2n} potentials, please set attractor_ic_scf to false");
    }
    class_test(pba->scf_parameters_size<5,
               errmsg,
               "For phi^2n potentials, all 5 scf_parameters must be set.");

    double fede = pba->scf_parameters[0];
    double n    = pba->scf_parameters[2];
    double phi_initial = sqrt(0.6667*fede*n*(2.0*n-1.0));

    pba->phi_ini_scf = phi_initial;
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_scf:
      ...
      /* FYCR-QDM */
      /* Adjust V_2 to get correct late time amout of dark energy*/
      else if (ba.scf_tuning_index == 3){
        xguess[index_guess] = 3.0*ba.Omega0_scf*pow(ba.H0,2);
        dxdy[index_guess] = 3.0*pow(ba.H0,2);
      }
      else{
